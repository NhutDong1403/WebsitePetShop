...
export const DanhMucBoSat = () => {
  const [tenDanhMuc, setTenDanhMuc] = useState("");
  const [danhSachDanhMuc, setDanhSachDanhMuc] = useState([]);
  const [message, setMessage] = useState("");
  const [messageType, setMessageType] = useState("");

  // Load danh mục từ API khi mở trang
  useEffect(() => {
    fetchCategories();
  }, []);

  // Lấy danh mục riêng bò sát
  const fetchCategories = async () => {
    try {
      const data = await getCategories("Pet", "Reptile"); // type = Pet
      ...
      <div className="max-w-7xl mx-auto mt-5 px-4 sm:px-6">
        <Breadcrumbs
          links={[
            ...
            { name: "Quản lý danh mục bò sát", to: "/Admin/danh-muc-bo-sat" },
          ]}
        />
      </div>

      <div className="max-w-4xl mx-auto mb-10 p-4 sm:p-6 border-2 border-black bg-lime-100 rounded-xl shadow-md mt-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-6 text-lime-800">
          Quản lý danh mục bò sát (Thú cưng)
        </h2>
        ...
        {danhSachDanhMuc.length === 0 ? (
          <p className="text-center text-xl text-black font-semibold mt-5">
          ...
          <ul className="space-y-3">
            {danhSachDanhMuc.map((dm) => (
              <li
                key={dm.categoryId}
                className="flex justify-between items-center px-4 py-2 bg-lime-200 border border-gray-200 rounded-lg shadow-sm"
              >
